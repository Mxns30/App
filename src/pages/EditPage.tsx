...
export default function EditPage() {
  ...
  const isChanged = useMemo(() => {
    ...
    // 이미지는 첫 장 기준과 개수 비교로 간단 체크
    const originalImages = (post.images && post.images.length > 0) ? post.images : (post.image ? [post.image] : []);
    const imagesChanged = originalImages.length !== croppedImages.length || originalImages.some((src, i) => src !== croppedImages[i]);
    ...
    <Container maxWidth="xs" sx={{ bgcolor: '#fafafa', minHeight: '100vh', pt: 8, pb: 8 }}>
      <Box sx={{ p: 3, pt: 2, position: 'relative' }}>
        ...
        <Button
          ...
          onClick={handleSave}
        >
          수정완료 <span style={{ marginLeft: 6 }}>🐾</span>
        </Button>
      </Box>
      {/* 이미지 크롭 다이얼로그 */}
