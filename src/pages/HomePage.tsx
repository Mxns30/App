...
import {
  Box, Container, Typography, TextField, IconButton, MenuItem, Button, Card, CardContent
} from '@mui/material';
...
import { collection, getDocs, query, orderBy, collectionGroup, deleteDoc, doc, getDoc, setDoc, serverTimestamp, updateDoc } from 'firebase/firestore';
...
const HomePage: React.FC = () => {
  ...
  const fetchFirebasePosts = React.useCallback(async () => {
    ...
    try {
      ...
      for (const doc of userPostsSnapshot.docs) {
        ...
        const includeBySchool = currentSchool ? (schoolFromDoc === currentSchool) : true;
        if (!includeBySchool) continue;
        allPosts.push({
          id: `${userId}_${doc.id}`, // 고유 ID 생성
          title: data.title || '제목 없음',
          school: schoolFromDoc || currentSchool || '한양여자대학교',
  ...
  const fetchLikes = React.useCallback(async () => {
    ...
    try {
      ...
      likesSnap.forEach(d => {
        ...
        if (currentSchool && data.school && data.school !== currentSchool) return;
        // key 규칙: sellerId_postId
        if (data?.sellerId && data?.postId) {
          next.add(`${data.sellerId}_${data.postId}`);
        } else {
          // 문서 ID가 sellerId_postId 규칙이면 그대로 사용
          next.add(d.id);
        }
      });
      setLikedIds(next);
  ...
  const handleRegister = async () => {
    ...
    if (editPostId) {
      // Firebase 게시글인지 확인
      const fb = firebasePosts.find(post => String(post.id) === String(editPostId));
      if (fb) {
        try {
          const [sellerId, docId] = String(editPostId).split('_');
          ...
          // 이미지가 선택되었으면 기존 이미지를 대체 업로드
          if (images.length > 0) {
            const folderRef = ref(storage, `posts/${sellerId}/${docId}`);
            try {
              const prev = await listAll(folderRef);
              await Promise.all(prev.items.map(item => deleteObject(item)));
            } catch {}

            await Promise.all(images.map(async (file, idx) => {
              const imgRef = ref(storage, `posts/${sellerId}/${docId}/image_${idx}.jpg`);
          ...
          console.error('🔥 Firebase 게시글 수정 실패:', e);
  ...
  // 찜(별) 토글 핸들러: Firebase 게시글은 Firestore likes 토글, 로컬은 로컬 토글
  const handleToggleLike = async (id: number | string) => {
    // Firebase 게시글인지 확인
    const firebasePost = firebasePosts.find(p => p.id === id);
    if (firebasePost && currentUser) {
      try {
        const [sellerId, docId] = firebasePost.id.toString().split('_');
        ...
        if (snap.exists()) {
          ...
          await setDoc(likeRef, {
            postId: docId,
            sellerId,
            postTitle: firebasePost.title,
            postPrice: firebasePost.price,
            postCategory: firebasePost.major,
            postImage: firebasePost.image,
            school: (firebasePost as any).school || currentSchool || null,
            createdAt: serverTimestamp(),
          });
          setLikedIds(prev => {
            const next = new Set(prev);
            next.add(`${sellerId}_${docId}`);
            return next;
          });
        }
        // UI 반영: 별 아이콘 색상은 MyPage에선 likes를 별도 로드하므로 여기선 별도 상태 안 둠
      } catch (e) {
  ...
  // 상세 페이지에서 넘어온 편집 요청 처리
  React.useEffect(() => {
    const state = location?.state as any;
    const targetId = state?.openEditForPostId;
    if (!targetId) return;

    const openEditor = (post: Post | undefined) => {
      if (!post) return;
      handleEdit(post);
      // state 초기화 (뒤로가기에 재오픈 방지)
      navigate(location.pathname, { replace: true });
    };

    // allPosts는 메모된 합산 목록이므로 여기에서 찾음
    const post = (allPosts as Post[]).find(p => String(p.id) === String(targetId));
    if (post) {
      openEditor(post);
    } else {
      // Firebase 로딩이 아직이라면 잠시 후 재시도
      const timer = setTimeout(() => {
        const p = (allPosts as Post[]).find(pp => String(pp.id) === String(targetId));
        if (p) openEditor(p);
      }, 400);
  ...
  return (
    <Container maxWidth="xs" sx={{ bgcolor: '#fafafa', minHeight: '100vh', pt: 8, pb: 8 }}>
      {/* 상단 */}
      <Box display="flex" alignItems="center" mb={isSearchOpen ? 1 : 2} gap={1}>
        <Typography fontWeight="bold" fontSize={18} sx={{ minWidth: 120 }}>
          {currentSchool}
        </Typography>
        <Box flex={1} />
        <IconButton size="small" onClick={handleToggleSearch} sx={{ color: '#1abc9c' }}>
      ...
      {/* 게시물 리스트 */}
      <Box>
        {searchTerm.trim().length > 0 && filteredPosts.length === 0 && (
        ...
        {filteredPosts
          .map((post) => (
              <Card key={post.id} sx={{ display: 'flex', mb: 2, boxShadow: 0, cursor: 'pointer' }} onClick={() => navigate(`/post/${post.id}`, { state: { from: '/' } })}>
              {post.image ? (
      ...
      >
        {menuPostId && (() => {
          ...
          <MenuItem onClick={() => { 
            const targetPost = allPosts.find(p => p.id === menuPostId);
            if (targetPost) {
              navigate(`/post/edit/${targetPost.id}`);
              handleMenuClose(); 
            }
          }}>
