...
import { collection, getDocs, doc, getDoc, collectionGroup, deleteDoc, setDoc, serverTimestamp } from 'firebase/firestore';
      ...
      try {
        ...
        for (const docSnapshot of userPostsSnapshot.docs) {
          ...
          if (firebasePostId === postId) {
            ...
            // 사용자 표시 이름 설정
            if (userId === myUid) {
              setUserDisplayName('나');
            } else {
              setUserDisplayName(`사용자 ${userId.slice(-4)}`); // UID 마지막 4자리로 표시
            }
            return;
          }
        }
        ...
        if (foundSamplePost) {
          ...
          // 샘플 데이터 사용자 표시 이름 설정
          setUserDisplayName(`사용자${foundSamplePost.userId.replace('sample_user_', '')}`);
          return;
        }
        ...
        if (foundPost) {
          ...
          if (foundPost.userId === myUid) {
            ...
            setUserDisplayName(`사용자 ${foundPost.userId.slice(-4)}`);
          }
          return;
        }
