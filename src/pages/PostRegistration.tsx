...
import {
  Box, Container, Typography, TextField, IconButton, Button, Dialog, DialogContent, Slider, CircularProgress
} from '@mui/material';
...
const PostRegistration: React.FC = () => {
  ...
  const isPriceValid = price.trim().length > 0 && Number(price) <= 10000000;
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const postRef = await addDoc(userPostsRef, {
        ...
        authorId: user.displayName || (user.email ? user.email.split('@')[0] : 'anonymous'),
      ...
      const uploadPromises = validImages.map(async (base64Image, index) => {
        const blob = base64ToBlob(base64Image);
        const storageRef = ref(storage, `posts/${user.uid}/${postRef.id}/image_${index}.jpg`);
    ...
    <Container maxWidth="xs" sx={{ bgcolor: '#fafafa', minHeight: '100vh', pt: 8, pb: 8 }}>
      <Box sx={{ p: 3, pt: 2, position: 'relative' }}>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ mt: -0.5, mb: 1, display: 'block' }}>
          10,000,000 원 이하로만 작성
        </Typography>
        {formSubmitted && !isPriceValid && (
          <Typography color="error" fontSize={13} mb={1}>
            {price.trim().length === 0 ? '필수 입력입니다' : '가격은 10,000,000원 이하만 입력 가능합니다'}
          </Typography>
        )}
        {/* 이미지 필수 안내 */}
        ...
        <Button
          ...
          variant="contained"
          sx={{
            background: (isFormValid && !isSubmitting) ? 'linear-gradient(135deg, #30D6A3, #5ED381)' : '#eee',
            color: (isFormValid && !isSubmitting) ? '#fff' : '#aaa',
            fontWeight: 'bold',
            borderRadius: 2,
            py: 1.2,
            fontSize: 18,
            mb: 1,
            mt: 1,
            boxShadow: (isFormValid && !isSubmitting) ? '0 4px 12px rgba(48, 214, 163, 0.3)' : 'none',
            '&:hover': {
              background: (isFormValid && !isSubmitting) ? 'linear-gradient(135deg, #5ED381, #30D6A3)' : '#eee',
              transform: (isFormValid && !isSubmitting) ? 'translateY(-2px)' : 'none',
              boxShadow: (isFormValid && !isSubmitting) ? '0 6px 16px rgba(48, 214, 163, 0.4)' : 'none',
            },
            cursor: 'pointer',
